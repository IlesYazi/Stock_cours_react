...
export default function Read() {
  ...
  const dbb = axios.get(`https://cosmic-raindrop-860c10.netlify.app/posts`).then((response) => {
    setDbbTaille(response.data.length);
  });

  useEffect(() => {
    axios
      .get(`https://cosmic-raindrop-860c10.netlify.app/posts?_page=1&_limit=2`)
      .then((response) => {
  ...
  const getData = (e, p) => {
    axios
      .get(`https://cosmic-raindrop-860c10.netlify.app/posts?q=${e}&_page=${p}&_limit=2`)
      .then((getData) => {
  ...
  const onDelete = (id) => {
    axios.delete(`https://cosmic-raindrop-860c10.netlify.app/posts/${id}`).then(() => {
      getData();
    });
  };

  const checkboxValue = (id) => {
    axios.get(`https://cosmic-raindrop-860c10.netlify.app/posts/${id}`).then((data) => {
  ...
  const majDataStock = (id) => {
    checkboxValue(id);
    axios.patch(`https://cosmic-raindrop-860c10.netlify.app/posts/${id}`, {
